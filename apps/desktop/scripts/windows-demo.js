...
/**
 * Runs the window-picker walkthrough (`pnpm demo:windows`).
 *
 * The walkthrough itself is TypeScript shared with the app
 * (`src/observation/demo.ts`), so the demo exercises the code that ships rather
...
try {
  const module = await server.ssrLoadModule('/src/observation/demo.ts');
  const result = await module.runObservationDemo();
