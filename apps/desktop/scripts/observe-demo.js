...
/**
 * Runs the observation walkthrough (`pnpm demo:observe`).
 *
 * The walkthrough itself is TypeScript shared with the app
 * (`src/observation/observe-demo.ts` over `src/observation/observe-rig.ts`), so
 ...
 * `packages/platform-mac` tests itself against. There is no macOS here
 * (runbook §5 amendment 8), so this is as close to the real capture path as
 * this machine can get.
 *
 * Usage:
 *   pnpm demo:observe
 */
...
try {
  const module = await server.ssrLoadModule('/src/observation/observe-demo.ts');
  const result = await module.runObserveDemo();
