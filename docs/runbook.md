...
the machinery to run each one. See follow-up 50 for whether this belongs in the
per-merge gate above; today it does not, because
`apps/desktop/test/acceptance/acceptance-suite.test.ts` runs the same thing
inside `pnpm test`.
