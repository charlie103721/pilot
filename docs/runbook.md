...
| `docs/release-candidate.md` | **Where MVP 01 stands** — the one document to read if you read one. What is built, what has never been verified, what the user must do, and what is waiting on their decision |
| `docs/known-issues.md` | Known issues, accepted exceptions, and the privacy/retention/stale-output finding, derived from §8's open follow-ups |
...
- **Node 24** — pinned in `.nvmrc` (`nvm use` on entry). Root `package.json`
  carries `"engines": {"node": ">=24 <25", "pnpm": ">=10.33 <11"}`; PR-044
  closed the upper bound, because Node 25 has never been run here.
...
| **The acceptance matrix and the grounding checklist, scored** — the last row of this table and the one every other row feeds. Needs the Mac **and** a model (row above, or §2), because 23 of the thirty grounding cases are about whether an *answer* describes the thing the pointer was on, and no answer has ever been scored anywhere. | `pnpm acceptance` (passes on Linux and reports **0 of 15 criteria verified**, 13 in part, 0 failed, 2 blocked-on-mac since PR-044 closed A-09 — it measures Pilot's *input* to the model and says so), then the six parts of `docs/handoff.md` §1 step 23: the thirty cases by hand against real applications, A-09's real behaviour, both display scales, and three timings including the one nobody has: key down → sound. | PR-043 | **yes — everything**, and it makes noise |
...
Acceptance evidence, added by PR-043 and runnable on Linux:

```sh
pnpm acceptance         # A-01…A-15 + thirty grounding cases, ~30 s
```

**This replaces the two static files this section used to promise**
(`docs/acceptance.md` and `docs/grounding-checklist.md`). A checked-in run log
goes stale the day after it is written and cannot be re-derived; an executable
harness reports the state of the tree it is run against. The run log *is* the
output of `pnpm acceptance`, and the checklist is
`apps/desktop/src/acceptance/grounding-cases.ts` — thirty cases as data, with
the machinery to run each one.

**It does not produce the ≥90% grounding figure and it says so before anything
else it prints.** That number is about a model's answers about a real screen and
neither exists here; `docs/handoff.md` §1 step 23 is the procedure that produces
it, and until it is run the honest state of §19's gate is *0 of 15 criteria
verified*.

### The release checklist (PR-044, and the answer to follow-up 50)

`pnpm acceptance` stays **out** of the per-merge gate and **in** this list.
`apps/desktop/test/acceptance/acceptance-suite.test.ts` already runs the same
harness inside `pnpm test`, so putting it in §6's five commands would pay ~30 s
and forty-odd helper processes per merge for evidence that is already green.
What it adds is the *printed verdict distribution*, and that is a thing a human
reads at a release, not at a merge.

Run all of this, in order, from a clean checkout before cutting a candidate:
...
pnpm test
pnpm build
pnpm package            # → apps/desktop/release/<platform>-unpacked
pnpm verify:bundle      # opens the produced archive
pnpm verify:package     # the macOS CONFIGURATION, never run on a Mac
pnpm smoke              # dev build, real Electron, headless
pnpm smoke:packaged     # the produced bundle
pnpm smoke:launch       # the produced bundle under `env -i`
pnpm acceptance         # READ THE DISTRIBUTION, not the exit code
pnpm demo:flow          # the MVP point-ask-hear scenario, end to end
pnpm demo:interrupt-flow && pnpm demo:memory && pnpm demo:local
pnpm demo:failure && pnpm demo:codex && pnpm demo:apikey && pnpm demo:privacy
```

`pnpm acceptance` exits 0 when no criterion has an *executed* check that did not
hold. **A zero exit is not a passing acceptance run** — a blocked criterion is
not a failure and a criterion nobody checked is not a pass. Read the banner.

### Producing the release artifact (PR-044)

Two artefacts exist and only one of them can be produced here.

**On Linux — a development bundle, and the only thing this project has ever
built.** `pnpm package` writes `apps/desktop/release/linux-unpacked/`. It is not
a distributable: Linux is a development host, not a supported target
(`electron-builder.yml`), and the staged helper is a **placeholder** because the
Swift helper links ScreenCaptureKit and can only be compiled on macOS. It
proves the packaging path, not the product. `verify-bundle.js` prints
`helper: PLACEHOLDER` so this can never be mistaken for the real thing.

**On macOS — the artefact.** Every step matters and the order matters; the two
places this most commonly goes wrong are marked.

```sh
nvm use                                        # Node 24
pnpm install --frozen-lockfile

# 1. Compile the Swift helper. NOTHING IN THIS PROJECT HAS EVER DONE THIS.
#    A failure here is a defect in packages/platform-mac/native/Sources —
#    send the compiler output; do not work around it.
swift build --package-path packages/platform-mac/native
swift test  --package-path packages/platform-mac/native

# 2. Stage the REAL helper. `--require-native` is the whole difference:
#    without it the build silently stages a placeholder and the bundle looks
#    identical. ← MISTAKE 1.
pnpm --filter @pilot/desktop run build:helper -- --require-native

# 3. Build, package, sign (ad hoc, inside the afterPack hook), and check.
pnpm package

# 4. Read the checks. `helper: native, … Info.plist embedded` is required;
#    `helper: PLACEHOLDER` means step 2 was skipped. ← MISTAKE 2.
pnpm verify:bundle
pnpm verify:package

# 5. Confirm what the signature actually sealed, which is the half a
#    successful `codesign` invocation does not tell you.
APP="apps/desktop/release/mac-$(uname -m | sed 's/x86_64/x64/;s/arm64/arm64/')/Pilot.app"
codesign --verify --verbose=2 "$APP"
codesign --display --entitlements - "$APP"                       # 3 entitlements
codesign --display --entitlements - "$APP/Contents/Resources/helper/PilotHelper"
                                                                  # exactly 1
otool -P "$APP/Contents/Resources/helper/PilotHelper" | head -40  # embedded plist

# 6. The distributable. `zip` is the only form that needs no certificate;
#    unzip into /Applications is what "install" means for an unsigned build.
ls apps/desktop/release/*.zip
shasum -a 256 apps/desktop/release/*.zip
```

The candidate is then: that `.zip`, its SHA-256, the commit it was built from,
the output of `pnpm verify:package`, and the `pnpm acceptance` distribution from
the same tree. Whoever receives it needs `xattr -dr com.apple.quarantine` or
right-click → Open, because it is not notarised —`docs/handoff.md` §1a is the
clean-machine sequence and `docs/known-issues.md` §1 is why.

**Do not ship one without `docs/handoff.md` §1 step 22 having been run at least
once.** Nothing above has ever executed; every check in step 5 is a check whose
expected output was reasoned about rather than observed.
...
### Phase 5 — COMPLETE on this machine (2026-08-11), and the gate is met only in part

All five PRs (040…044) are merged. **Read `docs/release-candidate.md` before
reading anything else in this section** — it is the clause-by-clause verdict on
the Phase 5 gate, and the short version is that the packaged application does
**not** meet the MVP definition of done, that the exceptions are enumerated and
accepted rather than hidden (`docs/known-issues.md` §1), and that the two
questions the product turns on have never been asked of a real model.

PR-040 ran alongside Phase 4 (PR-037…PR-039), which it does not touch: the
provider profiles own the profile surface, PR-040 owns failure handling
everywhere else, and the one place they meet is provider-neutral by
...
| 043 | **Acceptance and grounding suite.** The PR whose deliverable the plan states as "recorded acceptance results with at least 90% grounding accuracy", and whose first job was to establish that **no such number can be produced on this machine and that producing one anyway would be the worst outcome available**. Grounding accuracy is a property of a model's *answer* about a *real screen*; every provider here is a recorded fake or Pi's faux provider, so a percentage computed against one measures the script. What was built instead: `pnpm acceptance`, a harness that walks `docs/mvp-01-point-ask-hear.md` §18's A-01…A-15 with **a scenario per criterion** on the shipping composition and **derives** a verdict per row from the checks that ran — `verified` / `verified-in-part` / `failed` / `blocked-on-mac` / `blocked-on-model` / `not-implemented`. The rule is in `apps/desktop/src/acceptance/verdict.ts` and it is the point of the PR: no pass-condition check at all is `not-implemented`; none of them *executed* is `blocked-on-…` however much supporting evidence is green; an executed check that did not hold is `failed` and outranks both; and `executed()` throws on empty evidence so an unfalsifiable claim cannot be constructed. `verdict.test.ts` is that rule pinned in a millisecond, separately from the half-minute suite. Today: **0 verified, 12 verified in part, 1 failed, 2 blocked** — 45 pass-condition checks, 30 executed here, 15 waiting (10 on a Mac, 4 on a model, 1 on both), printed *before* anything else in the output. Thirty curated grounding cases run at **1× and 2×** — the first time the assembled application has run at anything but 2×, since `DEMO_DISPLAYS` is a single 2× display and `ask-demo.ts` hardcodes `scaleFactor: 2` — and each pins Pilot's *input*: the anchor's normalised point against §5's own arithmetic, inside/outside the window on §8's inclusive bound, the element retained or refused, the crop rectangle §10 step 5 computes, whether the thing under the pointer is **inside the picture the model receives**, and the envelope verbatim. 23 of the 30 report their verdict as pending a model; 7 are tool-contract cases and are decided here. **It found two things.** **A-09 fails** — Accessibility denied takes Pilot to `needs-permission` instead of §16's degraded visual mode, which is follow-up 35 demonstrated against the assembled application rather than merely recorded, and it is reported as `failed` rather than blocked so it cannot be filed with the missing machines. And the 1×/2× pairing surfaced follow-up 48, the scale-dependent crop extent. Follow-up 49 (the outside-window crop) is a question the pairing raised that only a model can answer. Latency spot checks are against system-design §17 with each budget's un-run half named: image preprocessing is the real pipeline on synthetic pixels on an idle Linux box (~70 ms median against a 150 ms budget), and the interruption number is Pilot's half only (~1 ms against 300 ms) because **nothing in this project has ever made a sound**. Raised follow-ups 48, 49 and 50; added 26 tests in 3 new files; no `packages/` change. `docs/handoff.md` §1 step 23. Demo: `pnpm acceptance`. |

| 044 (degraded grounding lane) | **Keep Pilot usable when Accessibility is denied** — the release-blocking defect PR-043 reported as A-09 `failed`, and follow-up 35 closed in its recorded shape. `REQUIRED_PERMISSIONS` is now `['screen-recording']`, so `restingState` stops resolving to `needs-permission` for the other three; the desktop's own catalogue has always said `screen-recording` is the only kind whose consequence is `blocks`, so **the fix was the machine agreeing with the panel rather than the panel changing**. All four readers of `permissionsAllowObservation` were re-checked and none needed a change (`App.tsx`'s `canConverse`, `WindowGate.allowsObservation()` with PR-041's `checking`-guard intact, `buildObservationView`, and the gate's own refusal path); a new test asserts the catalogue's `blocks` set and `REQUIRED_PERMISSIONS` are the same set so they cannot drift apart again. **The one `packages/` contract change** is `QuestionAnchor.targetAvailability` gaining `'unavailable'` beside `'reported'`/`'none'` (`@pilot/shared`), because "the hit test found nothing" and "no hit test was possible" are different facts and only the second is §16's; `renderAnchoredQuestionEnvelope` prints it as `pointer target: unavailable — Accessibility is not permitted, …` with a `reduced grounding:` instruction, and `QuestionEnvelopeRequest.accessibilityGrounding` (optional, `'unknown'` when omitted) is how the controller passes the machine's own permission snapshot in. `accessibilityGroundingOf` is **tri-state** — hazard 22 is the record of what reading one as a boolean costs. Two independent defences keep an element sampled *before* a revocation out of the model: the envelope refuses to summarise it, and `question-anchor.ts` refuses to attach it to the `observe_screen` result. `ObservationRuntime.notePermissions` drops the retained labels on the transition out of granted and **keeps the frames** — they are the degraded mode's remaining grounding — and `lifecycle-runtime.ts` stops arming `permission-loss` for a revocation that clears nothing. `LIFECYCLE_GUIDANCE['accessibility-revoked']` is now `recovered` with a sentence that says what got less reliable instead of announcing a stop. A-09 was **widened, not weakened**: six pass-condition checks execute where one did, and two are still pending (a real TCC revocation, and what a model does with a picture and a point). `pnpm acceptance` now reports `verified-in-part` and exits 0. Added 26 tests across 8 existing files; no new test file. `docs/handoff.md` §1 step 24. Demo: `pnpm acceptance`, `pnpm demo:failure` case 3. |
| 044 (model-identity lane) | **Say which model is answering, and warn when it is not one** — the second release-blocking defect, and the visible half of hazard 28. `ConversationGateState.modelStatus` and the `ModelStatus` row render for all four profiles; the development stand-in gets a `role="alert"` reading `NOT A REAL MODEL — answers are placeholder text` with the sentence "it is not a language model, it never sees your screen, and nothing it says about your screen is true", and a remedy naming the **absolute path** of `~/Library/Application Support/Pilot/pilot.env`. Each profile also states where screen images go, and the locality claim is allowed to err only towards *remote*: a loopback local endpoint reads "Local model on this Mac", a LAN address reads "Remote model — screen images are sent to …" in amber. `pnpm smoke:launch` asserts the faux-profile claim from the packaged bundle under `env -i`, so the hazard cannot recur silently. Closed follow-ups 33 and 46. **No human has ever seen the row** — `docs/handoff.md` §1 step 25 is two minutes of a user's time and it is the highest-consequence copy in the product. |
| 044 (release candidate) | **Freeze, document, package, certify.** No behaviour change: every direct dependency specifier is pinned exact (the resolved graph is byte-identical — the lockfile diff is 24 specifier lines and nothing else), `engines` gains an upper bound on Node and pnpm, `docs/known-issues.md` is written from this section's open follow-ups, §6 gains a release checklist (follow-up 50 closed), and `docs/release-candidate.md` is the clause-by-clause verdict against the Phase 5 gate. The gate is met **in part and explicitly**: the packaged application does not meet the MVP definition of done, and the deliverable is the enumerated list of exceptions rather than a claim that it does. Regression on this tree: 2 309 tests across 157 files, `pnpm lint`/`typecheck`/`build`/`package`/`smoke`/`smoke:packaged`/`smoke:launch`/`verify:package` green, `pnpm acceptance` exits 0 at 0 verified / 13 in part / 0 failed / 2 blocked-on-mac, and all eight walkthroughs green — including from a **fresh `git clone` with no `node_modules`**, which is this PR's demo. `packages/` change: none. |
...
28. **A double-clicked app has no environment, and every provider selector in
   ...
   **The rule this leaves**: a configuration mechanism that only exists on a
   ...
   that names the provider in use whichever profile it is — **is done**
   (follow-up 46, closed): `ConversationGateState.modelStatus` and the
   `ModelStatus` row render for all four profiles, and the faux one is a
   `role="alert"` reading `NOT A REAL MODEL — answers are placeholder text`
   with a remedy naming the launch file. `pnpm smoke:launch` now asserts that
   claim from the packaged bundle under `env -i`, so the hazard cannot recur
   silently — but note that **no human has seen the row**, which is the same
   sentence this hazard was written about. And note what a fix must not do: the file is an **allowlist**
...
| 33 | ~~**Nothing renders `describeEndpoint` to the user**…~~ **CLOSED by the model-status fix (PR-044's release-blocking lane), together with follow-up 46 — which is what the row asked for: one lane, once, additively.** `describeEndpoint` now reaches the panel through `src/conversation/model-status.ts`, a renderer-safe module that imports `@pilot/shared` and nothing else, so `EndpointDescription.label`/`detail` and the `isRemote` that **fails closed** are rendered as the Model row's `Screen images` line for **all four** profiles rather than for none. The route is the one the row named and the one PR-038 used — `ConversationGateState` → `apps/desktop/src/ipc/schemas.ts` → `src/conversation/view-model.ts` → `ConversationPanel.tsx` — and PR-038's `ModelDataDisclosure` is untouched and still rendered beside it. The fail-closed rule is asserted through the new surface (`local` profile, `isRemote: false`, `192.168.x.x` base URL → `Remote model — screen images are sent to …`, severity `attention`), and the *development* profile deliberately overrides the label: its base URL is `http://localhost:0`, so `describeEndpoint` would truthfully say "Local model on this Mac" about an address at which there is no model. It says `Nothing is sent anywhere: there is no model to send it to.` instead. | ~~the last of PR-037/038/039 to land~~ |
...
| 35 | ~~**Accessibility loss stops Pilot instead of degrading it** (PR-040; demonstrated as A-09 `failed` by PR-043).~~ **CLOSED by PR-044, in the recorded shape.** `REQUIRED_PERMISSIONS` is now `['screen-recording']` alone, so `restingState` no longer resolves to `needs-permission` for any of the other three. **The four callers, re-checked, and three of them needed no change**: `permissionsAllowObservation` has always answered `true` for Accessibility denied — the desktop catalogue classes it `degrades`, not `blocks` — so `App.tsx`'s `canConverse`, `WindowGate.allowsObservation()` (PR-041's `checking`-guard untouched) and `buildObservationView` were already right. **The gap was that the machine disagreed with them**: the panel offered the controls and the table refused every command. A new test asserts the two definitions are the same set, so they cannot drift apart again. **What the model is told** is the other half, and it is a `packages/shared` contract change: `QuestionAnchor.targetAvailability` gains `'unavailable'` beside `'reported'`/`'none'`, because "the hit test found nothing" and "no hit test was possible" are different facts and only the second is §16's. `renderAnchoredQuestionEnvelope` prints `pointer target: unavailable — Accessibility is not permitted, …` plus a `reduced grounding:` line telling the model to work from the picture and to say so; `PilotInteractionController` supplies it from the machine's own permission snapshot through `accessibilityGroundingOf`, which is tri-state (hazard 22: `unknown` is neither granted nor refused). Two independent defences drop an element read *before* a revocation — the envelope refuses to summarise it, `question-anchor.ts` refuses to attach it to the `observe_screen` result — and `ObservationRuntime.notePermissions` drops the retained labels while **keeping the frames**, which is the mode's remaining grounding. `LIFECYCLE_GUIDANCE['accessibility-revoked']` is now `recovered`, and `lifecycle-runtime.ts` no longer arms `permission-loss` for it (no clear follows a degradation, and an armed occasion would mislabel the next one). **The user sees** the existing `groundingDisclosure` and `grounding: 'reduced'` surfaces, unchanged, plus the new banner sentence. A-09 now reads `verified-in-part` — six pass-condition checks executed, two still pending on a Mac and on a model — and `pnpm acceptance` exits 0. | ~~PR-044~~ |
...
| 46 | ~~**Nothing on screen says which model Pilot is talking to, and in a packaged launch the answer is "none"**…~~ **CLOSED by the model-status fix (PR-044's release-blocking lane).** One provider-neutral IPC field (`ConversationGateState.modelStatus`) and one always-rendered row (`ModelStatus` in `ConversationPanel.tsx`), exactly as the row predicted. `main/index.ts`'s four-way `??` chain now yields a `ModelProfileKind` as data instead of a nested ternary spelled out twice, and `describeModelStatus` turns it into the sentence the panel shows, the line the startup log prints and the claim `pnpm smoke:launch` asserts — one wording, three surfaces. **The faux provider is the case it was built for and it does not read like a badge:** `NOT A REAL MODEL — answers are placeholder text`, `role="alert"`, its own `critical` severity, and a remedy naming only the variables `LAUNCH_ENV_ALLOWED` will actually accept (`PILOT_MODEL_PROFILE=codex`, `PILOT_LOCAL_BASE_URL=…`) plus the sentence saying why an API key cannot go in that file. `smoke:launch` — the packaged bundle under `env -i` — now fails if that claim is missing, so hazard 28 cannot come back silently. Liveness rides the subscriptions that already existed (`CodexRuntime.subscribe`, the API-key manager's `run-failed` handler), so a sign-in, a sign-out or a key that stops working changes the row without a relaunch and cannot disagree with `CodexStatus`. **No credential can reach it**: every string is a provider id, a model id, a host, a `userMessage` or fixed vocabulary, every one of them goes through `scrubUrlCredentials` (follow-up 42), no key name trips the redactor, and a test asserts `redactedPaths` is empty for all four profiles (hazard 25). Follow-ups **39** (no model picker) and **40** (`ModelProfileStore` unwired) are *not* closed by this and are still open: this is the surface that **says** which profile is in force, not one that lets the user change it. **Never seen by a human** — see `docs/handoff.md` §1 step 25. | ~~PR-044~~ |
...
| 48 | **The pointer crop covers a different amount of the window at 1× than at 2×** (PR-043). `MVP_SCREEN_POLICY.pointerCropPixels` is 640 **captured** pixels, and the captured image is not the same size at the two scales: `capture.start` asks for 1200×800 px for a 1200×800 pt window at 1× and 1440×960 px at 2× (2400×1600 reduced by `fullFrameMaxEdge`). So the same gesture hands the model a crop covering 640 pt of window on a standard display and about 533 pt on a Retina one — and, if `fullFrameMaxEdge` ever rises, 320 pt. `pnpm acceptance` §4 prints both, read off the same ten pointer positions run at both scales. Nothing is *wrong* at either scale, no test fails, and the geometry is correct throughout — §5 converts through `captureSize` rather than `scaleFactor` precisely so a policy-downscaled capture converts right. It is a **policy** question §10 never asked: should the crop be a constant fraction of the *window* rather than a constant number of pixels? Deliberately not changed here — PR-043 is an acceptance PR and changing the shipped crop size would have made every earlier walkthrough's byte counts move for a reason unrelated to acceptance. Only a real model can say whether the tighter Retina crop loses context it needed: `docs/handoff.md` §1 step 23 (d). | PR-044 |
| 49 | **A pointer outside the selected window still produces a pointer crop** (PR-043). `docs/mvp-01-point-ask-hear.md` §8 says that when the pointer is outside the selected window Pilot "reports that state to the model and does not invent a target". It does not invent a *target* — the anchor's element is discarded, the envelope reads `pointer: 1.167, 1.087 (window-relative) — outside the selected window; no element was identified`, and grounding cases G-10 and G-20 check that the real element sitting there reaches the prompt nowhere. But `view: 'both'` still returns a **picture**: `pointerCropRect` clamps the centre into the frame, so the model receives a 640×640 close-up of the window corner nearest the pointer, labelled `pointer`. The model is told the pointer was outside, so this is arguably correct — a crop of *somewhere* plus an accurate caption — and it is arguably a picture of a place the user was not looking at. **Nobody has ever seen a model read that pair**, which is exactly why this is a row and not a fix. `docs/handoff.md` §1 step 23 (b) asks for the outside-window question three times and for whether the answer ever describes the corner. | PR-044 |
| 50 | **`pnpm acceptance` is not in `docs/runbook.md` §6's gate, and deciding that is PR-044's** (PR-043). It takes about 30 s, spawns forty-odd helper processes and is already covered by `apps/desktop/test/acceptance/acceptance-suite.test.ts` inside `pnpm test`, so running it again in §6 would double the cost for the same evidence. What §6 *would* gain is the printed output — the verdict distribution is the thing a human reads and the test only asserts it exists. The middle option is to add it to the release checklist rather than to the per-merge gate. **CLOSED by PR-044, and the middle option is the answer**: §6 gains a *release checklist* that runs `pnpm acceptance` and all eight walkthroughs from a clean checkout, and the five-command per-merge gate is unchanged. The reasoning is that a verdict distribution is read by a human at a release and by nobody at a merge, while the assertion that it exists is worth having on every merge — which is what `acceptance-suite.test.ts` already does. | ~~PR-044~~ |
