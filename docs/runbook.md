...
| 14 | **A new question during `observe_screen` steers the old run *and* submits a new one.** PR-006 chose `steer` in `observing-screen` so a capture can unwind (§15), but `steer` does not end the run: with the real `PiAgentSession`, the `submit-question` that follows in the same transition hits `run-already-active` and surfaces as a user-visible error ("Pilot is still working on the previous question"). It recovers — the failure teardown aborts the steered run — but the interruption did not do what the user asked. The fakes now model this faithfully (`InterruptibleAgentSession`), and `test/interruption.test.ts` pins the current behaviour so it cannot drift silently. The fix is a design decision PR-027 declined to take alone: either deliver the new question *as* the steering message (keeping `activeRunId`, no new run), or abort instead of steering once a replacement question exists. **PR-034 checked and deliberately did not touch it.** Its trace interrupts in `speaking`, which is `interruptModeFor`'s `abort` branch and is clean: the synthesiser is told to stop, no chunk of the abandoned answer reaches it afterwards, and the follow-up is asked and answered on the same conversation. The `observing-screen` case — the one this row is about — is the harder half and stays PR-035's, along with the design decision. | PR-035 |
...
| 31 | **`ObservationRuntimeMetrics.pointerSamples` is always 0 on the shipping path** (PR-034). `main/observation-runtime.ts` samples the pointer through `MacAccessibilityAdapter.groundFast` (PR-013's one-round-trip form, chosen by PR-028 for the 30 Hz cadence) and ingests the result straight into `ObservationCore`; only the `session.samplePointer()` fallback — the path a platform with neither `ground` nor `groundFast` takes — increments `ObservationSession.#pointerSamples`, which is what the metric reports. So the app's own count of pointer samples reads 0 however many were taken, and `pnpm demo:flow` counts `accessibility.sample` on the wire instead. Nothing consumes the metric today, which is why it has cost nothing; the moment the diagnostics panel shows it (PR-036's telemetry, or PR-041) it is a wrong number rather than a missing one. Fix is one line in `samplePointer` or a counter on the runtime beside `starts`/`stops`. | PR-036 or PR-041 |
