...
chooses them in one branch and neither is usable without the other. Speech
*output*, the model and persistence are still fake, each named in the table at
the top of `apps/desktop/src/main/index.ts` with the PR that takes it. **The model is still
