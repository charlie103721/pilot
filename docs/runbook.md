...
| PR | What landed |
| --- | --- |
| 028 | Observe a real selected window (needs the Mac) |
...
PR-029 replaced exactly one fake boundary — the agent. Observation, speech,
permissions, the window list and persistence are all still fake, each named in
the table at the top of `apps/desktop/src/main/index.ts` with the PR that takes
it. **The model is still faux** (`docs/handoff.md` §2): no sign-in has happened,
so nothing has ever talked to a real provider.
...
10. **A "fake" that only moves when a test tells it to is wrong in an app.**
   ...
   still *finishes* on its own before wiring it. See follow-up 24.
...
| 16 | **The app must supply real permission states to `ScreenContextService`** (PR-019). `ScreenContextConditions.permissions` defaults to `'unknown'`, which §10 step 1 refuses with `permission-denied` — deliberately, so an unwired facade fails loudly instead of proceeding on a guessed grant. PR-028 must feed `PermissionAdapter` (and PR-011's attribution check) into `MutableScreenContextInputs.setConditions`, along with `paused` and `enabled` from the interaction controller. Until then every observation is refused. | PR-028 |
| 17 | **Lifecycle events must route through `RetentionGuard.clearFor` (or `ScreenContextService.clear(event)`), not `ObservationSession` alone** (PR-019). The session clears the core directly on window loss and screen lock; that path empties the ring but is not the guard, so it does not reset the rate limiter or drop the decoded-frame cache. The facade notices at the next observation and drops the cache then, which is a backstop rather than the door. Wire pause / lock / window-loss / shutdown to the guard and the two agree at the moment they happen. | PR-028 |
| 18 | **Capture should hand over `bgra` or `png`, not `jpeg`** (PR-018). No contract change: `FrameEncoding` already admits all three and `CaptureOptions` says nothing about encoding. Delivering an encoded JPEG costs ~165 ms of pure-JS decode per observation that needs a pointer crop (the only path over §17's 150 ms budget) and adds a second generation of compression loss to exactly the small text grounding depends on. `png` fits the 16 MiB ring ceiling where `bgra` does not; `bgra` is the right choice for the fresh `captureFresh` path, which does not enter the ring. Measured in `pnpm --filter @pilot/observation demo:image` §5. | PR-012, confirmed in PR-028 |
...
| 23 | **The two mocked ports PR-029 left, and where they attach** (PR-029). `createAgentRuntime({ screenContext })` already takes a `ScreenContextService` and defaults to `FakeScreenContextService`, so **PR-030 passes the real one and changes nothing else** on the agent side. `createInteractionRuntime({ observation })` takes an `ObservationControlPort` and defaults to `createMockObservationControlPort`, which records calls and captures nothing; **PR-028 passes the real capture lifecycle there**. Both defaults record rather than ignore, so "Look now completed" today means the machine's path ran, never that a screen was read. | PR-028 / PR-030 |
