...
> **If you are the user and you read one document, read
> `docs/release-candidate.md`.** It is where MVP 01 stands, what has never been
> verified, what only you can do, and every decision waiting on you with a
> recommendation attached. This file is the detail behind it: §1 is the Mac
> batch as runnable commands, §2 is the Codex sign-in, §3 the accepted gaps and
> §4 the decisions taken on your behalf.
...
| The packaged app cannot be pointed at a real provider from its own UI | Provider selection is by environment variable, and Finder supplies none. PR-042 added a launch environment file (`~/Library/Application Support/Pilot/pilot.env`) so a double-clicked Pilot can be configured without a terminal, but there is still no model picker in the panel. **Half of this is closed**: PR-044's model-identity lane made the panel say which provider is in use in all four profiles, where the screen images go, and — for the development stand-in a Finder launch falls through to — that it is not a language model at all (follow-ups 33 and 46, closed). What remains is that there is **no picker**: the choice is still an environment variable or a line in `pilot.env`, and `ModelProfileStore` is still unwired (follow-ups 39 and 40). `docs/known-issues.md` KI-02. | accepted for MVP 01 |
...
| Phase 5 — hardening and release (040…044) | **COMPLETE on this machine, and the honest sentence is that "complete" means the work is done rather than that the product is proved. PR-044 is merged: versions frozen, known issues written down, the artefact produced as far as a Linux box can produce it, and the Phase 5 gate answered clause by clause in `docs/release-candidate.md`.** The two release-blocking defects were fixed in their own lanes first (degraded grounding; the model-identity row). This lane changed no behaviour: every direct dependency specifier is now exact and the resolved graph is unchanged, `engines` gained an upper bound, `docs/known-issues.md` accounts for every open follow-up, and §6 of the runbook gained a release checklist (follow-up 50, closed). The gate's first clause is discharged as **"documents an explicitly accepted exception"** and there are eight of them; the second (all acceptance evidence committed) holds and re-runs from a clean clone; the third (no known privacy, retention or unsafe-stale-output defect) holds, with the audit's own limits stated rather than assumed. **What has not changed at all: there is still no Mac, no compiled helper, no model, no microphone, no speaker and no pixel.** `docs/release-candidate.md` is the document to read. |

Last full regression on `main` (PR-044, the release candidate): **2 309 tests
across 157 files**, `pnpm lint`, `typecheck`, `build`, `package`,
`verify:package`, `smoke`, `smoke:packaged` and `smoke:launch` all green,
`pnpm acceptance` exiting 0 at *0 verified / 13 verified-in-part / 0 failed /
2 blocked-on-mac*, and **all eight walkthroughs green** — `demo:flow`,
`demo:interrupt-flow`, `demo:memory`, `demo:local`, `demo:failure`,
`demo:codex`, `demo:apikey`, `demo:privacy` — including from a fresh `git clone`
with no `node_modules`. (The earlier figure recorded here, 997 tests across 66
files after PR-017/PR-022a, was two phases stale.)
